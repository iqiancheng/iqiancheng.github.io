...
- HoPE 的"时间轴零频率"看起来和 MRoPE-I 的"每个轴都有全频谱"矛盾，其实不矛盾：MRoPE-I 说"每个轴要有访问全频谱的**权利**"，HoPE 说"时间轴在整个频谱里应该**选零**"。两者可以组合——在 MRoPE-I 的轮询框架下，把时间通道的 $$\theta$$ 设为 0。
...
**3. 时间轴可能该退化为 NoPE。** HoPE 的定理（$$\theta=0$$ 最优）在 ASR 场景可能同样成立——音频帧之间的相对位置应由内容决定，而非 RoPE 旋转角。如果 MTP 的验收目标是采样接受率，时间位置编码引入的噪声可能比信号多。
